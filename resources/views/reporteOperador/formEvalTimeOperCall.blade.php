...
use GuzzleHttp\Client;
        $form_data = array('route' => 'reporteOperador.store', 'method' => 'POST');
        $action    = 'Crear';     
...
<h1>{!! $action !!} reporte evaluación del tiempo del operador por llamada</h1>

{!! Form::model($reporteOperador, $form_data, array('role' => 'form')) !!}
  ...
  <div class="row">
    ...
    <input type='hidden' id='tipo' name='tipo' value='evalTimeOperCall' />
...
function dest() {
    $.ajax({
     url:'http://localhost/callcenter/public/campaignOperator',//<?php echo  url('/call/to'). "/" ?>
      type:"GET",
    ...
    success:function(data){  
      ...
      var html_str ='';
       ...
       for(var i=0; i<data.length; i++)
       {
        console.log(data);
         console.log(data[i]);
       html_str += '<td><input type="checkbox" id="grupoOperador" name="grupoOperador[]" checked value="'+ data[i].idPg +'">'+ data[i].name +'</td>';
       }
       //e.preventDefault();
       //html_str +='</ul>';
