				...
				@else
					@foreach ($questionlist as $ql)
						...
						@if($campaign->active=='true')
							<td> <a disabled href="#" style="text-decoration:none;">{{ $ql->data }} | {{ $ql->order }} </a>
							...
							<td> 
					 		@if($ql->idQuestionType==10)
								...
								<a disabled href="" style="text-decoration:none; border:none;">
							...
							<td> 
							 		@if($ql->idQuestionType==10)
							 			<?php
											...
											//dd($questionDestinatary);
										?>    
							 			</a><a class="show-option" title="condición a la pregunta {{ $ql->data }}, en la posición {{ $questionC->order }}, de tipo {{ $questionTypeCond->name }}" disabled>Condición {{ $ql->data }} </a>
							 		@elseif($ql->idQuestionType==9)
