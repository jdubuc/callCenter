...
<?php
    if ($campaign->exists):
        $form_data = array('route' => array('campaignEncuesta.update', $campaign->id), 'method' => 'PATCH');
        $action    = 'Editar';
        $campaign->idPersonModificator = Auth::user()->id;
    else:
        $form_data = array('route' => 'campaignEncuesta.store', 'method' => 'POST');
        $action    = 'Crear';     
        $campaign->idPersonModificator = Auth::user()->id;
...
<h1>{!! $action !!} campaign encuesta</h1>
